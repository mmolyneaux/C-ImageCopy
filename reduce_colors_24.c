...
void reduce_colors_24(Image_Data *img) {
    ...
    int palette_size = (img->output_color_count < pallet_size_max)? img->output_color_count : pallet_size_max;
    ...
    for (int i = 0; i < npixels; i++) {
        RGB c = pixels[i];
        if (c.r < mn.r) mn.r = c.r; if (c.g < mn.g) mn.g = c.g; if (c.b < mn.b) mn.b = c.b;
        if (c.r > mx.r) mx.r = c.r; if (c.g > mx.g) mx.g = c.g; if (c.b > mx.b) mx.b = c.b;
    }
    boxes[0].min = mn;  boxes[0].max = mx;
...
// #pragma pack(pop)
    ...
    for (int i = nboxes; i < 256; i++) {
        fputc(0, fo); fputc(0, fo); fputc(0, fo); fputc(0, fo);
    }
    ...
    for (int y = height-1; y >= 0; y--) {
        ...
        for (size_t p = width; p < oprow; p++) fputc(0, fo);
    }
    fclose(fo);
    free(indices);
...
void median_cut(RGB *pixels, int np, Box *boxes, int *nb, int target) {
    while (*nb < target) {
        ...
        if (dr >= dg && dr >= db) {
            qsort(pixels + b.start, len, sizeof(RGB),
                  (int(*)(const void*,const void*))[](const RGB*a,const RGB*b){
                    return a->r - b->r; });
        } else if (dg >= dr && dg >= db) {
            qsort(pixels + b.start, len, sizeof(RGB),
                  (int(*)(const void*,const void*))[](const RGB*a,const RGB*b){
        ...
        for (int j = boxes[*nb].start; j < boxes[*nb].end; j++) {
            RGB c = pixels[j];
            if (c.r < boxes[*nb].min.r) boxes[*nb].min.r = c.r;
            if (c.g < boxes[*nb].min.g) boxes[*nb].min.g = c.g;
            if (c.b < boxes[*nb].min.b) boxes[*nb].min.b = c.b;
            if (c.r > boxes[*nb].max.r) boxes[*nb].max.r = c.r;
            if (c.g > boxes[*nb].max.g) boxes[*nb].max.g = c.g;
            if (c.b > boxes[*nb].max.b) boxes[*nb].max.b = c.b;
        }
        (*nb)++;
    }
}
...
void apply_dither(RGB *img, int w, int h,
                  RGB *pal, int psize, uint8_t *out)
{
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            int idx = y*w + x;
